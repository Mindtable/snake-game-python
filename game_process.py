...
def print_text(screen, text, x, y, font_size=24):
    font1 = pygame.font.SysFont('serif', font_size)
    to_print = font1.render(text, True, white)
    screen.blit(to_print, (x, y))


def game_process(screen, game_zone_w, screen_height, fps, clock):
    global point
    ...
    while not done:
        ...
        if not counter:
            ...
            if len(stop) < len(body):
                death_screen(screen, game_zone_w, fps, clock, point)
            ...
            screen.fill((0, 0, 0))
            for part in body:
                pygame.draw.rect(screen, (255, 0, 0), [part[0], part[1], 20, 20])
            pygame.draw.rect(screen, (255, 255, 255), [food_position[0], food_position[1], 20, 20])
            print_text(screen, 'Your score: ' + str(point), 520, 20, 24)
